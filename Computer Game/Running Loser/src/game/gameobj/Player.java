...
public class Player extends GameObject implements CommandSolver.KeyListener {
    ...
    private Delay effectDelay; // buff持續的delay
    private int downSpeed;  // 減速
    private boolean isChange;//交換位置
    private Delay paintDelay;//無敵閃爍
    private boolean paint;//是否閃爍
    private boolean isChaos; // 是否混亂
    //火球術
    private boolean canShot;
    private Delay fireBallDelay; // 技能cd
    //ID
    private int Id;
    //effect
    private Image imgInvincible; // 無敵
    private Image imgSpeedUp;  // 加速
    private Image imgSpeedDown; // 減速
    private Image imgChaos;  // 混亂

    public Player(int x, int y) {        // 可有可無
        this(ActorType.SASUKEHD, x, y);
    }

    public Player(ActorType actorType, int x, int y) {
        super(x, y, ACTOR_WIDTH, ACTOR_HEIGHT);
        dir = Direction.RIGHT; // 初始向右
        animator = new PlayerAnimator(State.JUMP, actorType); // 預設跳躍動畫
        this.Speed = new Vector(0, 0);      // 預設速度0
        this.isJumping = true;    // 跳躍中(布林)
        isSquating = false;  // 非蹲下中(布林)
        this.ySpeed = 0;  // 無重力加速度
        this.actorType = actorType;   // 外面給定腳色
        this.rebornPoint = new int[2];  // 重生點座標                     可改
        this.rebornPoint[0] = x;      // 
        this.rebornPoint[1] = y;   // 
        this.deathDelay = new Delay(45);  // 死亡delay 45幀
        this.deathDelay.stop();   // 先停用死亡
        this.coinCount = 0;      // 金幣計數起始0
        this.canGetEffect = true;  // 預設可得到技能
        this.skillDelay = new Delay(PLAYER_EFFECT_CD);  // buff的cd
        this.skillDelay.stop();                          
        this.extraSpeed = 0;                            // 預設無加速度
        this.isInvincible = false;                      // 預設非無敵
        this.paintDelay = new Delay(3);  //無敵狀態閃爍  
        this.paint = true;         // 閃爍(布林)
        this.canShot = true;       // 可射擊(布林)
        this.fireBallDelay = new Delay(PLAYER_SKILL_CD);  // 技能cd
        this.effectDelay = new Delay(EFFECT_DELAY);      // buff的持續
    ...
    public void setJumping(boolean jumping) { // 外面設定是否跳躍狀態
        this.isJumping = jumping;
    }

    public void setCanGetEffect(boolean canGetSkill) {  // 外面設定是否可撞?磚
    ...
    public Delay getSkillDelay() {  // 取得可撞?磚的delay
    ...
    public void setMonsterCount(int monsterCount) {       // 外面設定殺怪數
        this.monsterCount = monsterCount;
    }

    public final void setSpeed(Vector vector) {          // 外面設定速度向量
        this.Speed = vector;
    }

    public void setState(State state) {      // 設定動作狀態
        this.animator.setState(state);
    }

    public void setDir(Direction dir) {      // 設定方向
        this.dir = dir;
    }
    ...
    public void move() {//移動
        if (!outOfScreen() && animator.State() != State.DEAD) {  // 若未出螢幕，且未死亡
            translateX((int) Speed.vx()); // x移動 speed向量取x分量
    ...
    public void chaos() {  // 混亂
        this.isChaos = true;
        this.effectDelay.play();
    }

    public void effect(int effect) { // 正向buff
        switch (effect) {
            case 1://加速
                this.extraSpeed = PLAYER_SPEED;
                break;
            case 2://無敵
