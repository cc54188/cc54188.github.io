...
public class PlayScene extends Scene {
    ...
    public void sceneBegin() {
        ...
        p1Backgrounds.add(new Background(BACKGROUND_BOTTOM_WIDTH / 2, BACKGROUND_BOTTOM_HEIGHT / 2, BACKGROUND_BOTTOM_WIDTH, BACKGROUND_BOTTOM_HEIGHT, Background.BackgroundType.BOTTOM)); // 背景底層
        p1Backgrounds.add(new Background(BACKGROUND_BOTTOM_WIDTH / 2 * 3, BACKGROUND_BOTTOM_HEIGHT / 2, BACKGROUND_BOTTOM_WIDTH, BACKGROUND_BOTTOM_HEIGHT, Background.BackgroundType.BOTTOM)); // 接在右邊
        p1Backgrounds.add(new Background(BACKGROUND_MIDDLE_WIDTH / 2, BACKGROUND_MIDDLE_HEIGHT / 2, BACKGROUND_MIDDLE_WIDTH, BACKGROUND_MIDDLE_HEIGHT, Background.BackgroundType.MIDDLE));  // 背景中層
        p1Backgrounds.add(new Background(BACKGROUND_MIDDLE_WIDTH / 2 * 3 - 10, BACKGROUND_MIDDLE_HEIGHT / 2, BACKGROUND_MIDDLE_WIDTH, BACKGROUND_MIDDLE_HEIGHT, Background.BackgroundType.MIDDLE));  // 接在右邊
        ...
        //計分
        pointCount = SceneController.instance().irc().tryGetImage(new Path().img().backgrounds().pointCount()); // 記分板
        ...
        cameraArr.add(new Camera.Builder(SCREEN_X, SCREEN_Y / 2)// 創建一個半螢幕大小的鏡頭
        ...
        // PlayScene內的介面
        coinCount = new CoinAnimator(CoinType.COINCOUNT); //建構計算金幣的圖
        monsterCount = new MonsterAnimator(MonsterType.PIG); //建構計算怪物的圖
        
        mapInitialize();  // 地圖載入器方法
//        AudioResourceController.getInstance().loop("resources\\sounds\\background-2.wav", 99);
        AudioResourceController.getInstance().loop(new Path().sound().background(), 99); // 放音樂
    }

    @Override
    public void sceneEnd() {  // 也許要更多null
        ...
        for (int i = 0; i < cameraArr.size(); i++) {
            ...
            if (i == 0) { // p1背景
                for (int j = 0; j < p1Backgrounds.size(); j++) {
                    if (cameraArr.get(i).isCollision(p1Backgrounds.get(j))) { // 若p1鏡頭碰撞p1背景
                        p1Backgrounds.get(j).paint(g);      
                    }                // 畫出順序: 底1->底2->中1->中2
                }
            } else { // (i == 1) p2背景
                for (int j = 0; j < p2Backgrounds.size(); j++) {
                    if (cameraArr.get(i).isCollision(p2Backgrounds.get(j))) { // 若p2鏡頭碰撞p2背景
            ...
            cameraArr.get(i).end(g); // 以上為畫在鏡頭內的東西
        }
        g.fillRect(0, SCREEN_Y / 2, SCREEN_X, 5); // 雙鏡頭分隔線
        //繪製距離
        g.drawImage(pointCountOrange, DISTANCE_X - 50, DISTANCE_Y - 40, 350, 60, null);  // p1
        g.setColor(Color.WHITE);
        g.setFont(new Font("Dialog", Font.BOLD, 30)); // 字形
        g.drawString(/*"残り時間:" + passedTime + "\t秒" +*/"走る距離: " + players.get(0).collider().right() / 10 + " M", DISTANCE_X, DISTANCE_Y);
        g.drawImage(pointCount, DISTANCE_X - 50, SCREEN_Y - 60, 350, 60, null); // p2
        g.drawString(/*"残り時間:" + passedTime + "\t秒"+*/"走る距離: " + players.get(1).collider().right() / 10 + " M", DISTANCE_X, SCREEN_Y - 20);
        //繪製金幣獲得數量-------->在不同電腦上會顯示不同位置
        coinCount.paint(COINCOUNT_X, COINCOUNT_Y, COINCOUNT_X + 41, COINCOUNT_Y + 48, g);  // p1
        monsterCount.paint(MonsterType.PIG, Direction.LEFT, COINCOUNT_X, MONSTERCOUNT_Y, COINCOUNT_X + 41, MONSTERCOUNT_Y + 48, g); // 打怪數圖示
        g.setColor(Color.WHITE); // 可刪
//        g.setFont(new Font("Dialog", Font.BOLD, 40));
        g.drawString("X" + players.get(0).getCoinCount(), COINCOUNT_X + 41, COINCOUNT_Y + 40);  // 顯示已吃金幣數
        g.drawString("X" + players.get(0).getMonsterCount(), COINCOUNT_X + 41, MONSTERCOUNT_Y + 40); // 顯示打怪數
        coinCount.paint(COINCOUNT_X, SCREEN_Y - 54, COINCOUNT_X + 41, +SCREEN_Y - 6, g);    // p2
